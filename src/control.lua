...
function onputitem(event)
	if isHolding("staff-of-teleportation", event.player_index) then
		...
		if player.cursor_stack.valid_for_read and player.cursor_stack.name=="staff-of-teleportation" and cursor_stack.can_set_stack then
...
function moveFast(targetpos, player_index)
    ...
    local safe_pos = player.surface.find_non_colliding_position(player.character.name, telepos, 15, .1)
    if safe_pos then
        player.teleport({x = safe_pos.x, y = safe_pos.y})
    end
end
